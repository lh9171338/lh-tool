import asyncio
from src.lh_tool.Iterator import (
    SingleProcess,
    MultiProcess,
    AsyncProcess,
    AsyncMultiProcess,
...
def test_iterator():
    ...
    result_list = AsyncMultiProcess(async_process, nprocs=10).run(
        a=a, b=b, opt="+"
    )
    print(result_list)

    ret_list = ParallelProcess(process_batch, nprocs=5).run(
        a_list=a, b_list=b, opt="+"
    )
    result_list = []
    for ret in ret_list:
