...
def images2video(
    image_path, video_file, postfix, fourcc, fps, frameSize=None, speed=1
):
    image_file_list = sorted(
        glob.glob(os.path.join(image_path, f"*.{postfix}"))
    )
    if len(image_file_list) == 0:
        return
    video_file = (
        os.path.abspath(image_path) + ".mp4"
    ...
    image_file_list = [
        image_file_list[i] for i in range(0, len(image_file_list), speed)
    ]
...
def main():
    ...
    parser.add_argument(
        "-s", "--size", type=int, nargs=2, help="desired frame size for video"
    )
    parser.add_argument(
        "-a", "--speed", type=int, default=1, help="speed for video"
    )
    parser.add_argument(
        "-r",
        "--recursive",
    ...
    try:
        ...
        if recursive:
            ...
            images2video(
                image_path, video_file, postfix, fourcc, fps, size, speed
            )
