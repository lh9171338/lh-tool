...
def concat_image(
    image_file_list, output_file, padding_size=5, hstack=True, black=True
):
    assert len(
        image_file_list
    ), "The number of image files must greater than 0"
        ...
        if image is None:
            ...
            if hstack:
                input_image = cv2.resize(
                    input_image, (round(w * height / h), height)
                )
                image = np.hstack((image, padding, input_image))
            else:
                input_image = cv2.resize(
                    input_image, (width, round(h * width / w))
                )
...
def main():
    ...
    parser.add_argument(
        "-c", "--cover", action="store_true", help="cover the input image file"
    )
    parser.add_argument(
        "-s", "--padding_size", type=int, default=5, help="padding size"
    )
    parser.add_argument(
        "-v",
        "--vertical",
    ...
    try:
        ...
        if recursive:
            image_file_list = sorted(
                glob.glob(os.path.join(opts.input, f"*.{postfix}"))
            )
            another_image_file_list = sorted(
                glob.glob(os.path.join(opts.another_input, f"*.{postfix}"))
            )
            ...
            output_file_list = [
                os.path.join(output_path, os.path.basename(image_file))
                for image_file in image_file_list
            ]
            if nprocs == 1:
            ...
            if os.path.isdir(opts.input):
                ...
                output_file = (
                    os.path.abspath(opts.input) + f".{postfix}"
                    if opts.output is None
                    else opts.output
                )
                concat_image(
                    image_file_list, output_file, padding_size, hstack, black
                )
            else:
                ...
                assert (
                    cover or output_file is not None
                ), "Output file is 'None'"
